"""[일회용] 2026-08-08 3차 재검증 - 화이트리스트 ⭐⭐ 를 작성자 줄에서
분리한 수정 확인용 샘플 재발송.

앞선 두 차례 샘플에서 '✍️ 작성자: @mastercrypto2020 ⭐⭐' 줄이 계산상
예산 이내인데도 실제로는 계속 줄내림되는 것을 스크린샷으로 확인했다.
예산을 낮추는 접근을 포기하고 별표를 별도 줄로 분리 - 어떤 사용자명
길이에서도 별표가 줄내림을 유발할 수 없는 구조로 바꿨다. 이번엔 그
결과를 실제로 확인한다. [샘플] 표식 명시. 확인 후 워크플로와 함께
삭제 예정.
"""
import sys
...
def main():
    ...
    touch_msg = ("🧪 <b>[샘플 3차 재검증 - 별표 줄 분리 확인]</b>\n" + touch_msg)
