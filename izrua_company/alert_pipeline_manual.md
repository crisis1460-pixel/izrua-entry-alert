...
| 방향 | 설정 키 | 기본값 | 의미 |
|------|---------|--------|------|
| `dev > 0` 진입가가 현재가 **위** | `watch_entry_max_above_pct` | **300%** (2026-09-14 상향, 감사 F2) | 롱 대기인데 진입가가 이미 지나간 자리 = 오파싱 탐지축. **60% 는 정상 급락을 오인했다** — 이 방어선이 터치 루프보다 앞이라, 회차가 길게 끊긴 뒤 가격이 진입가 아래로 크게 빠진 레벨이 '소급 터치'가 아니라 만료가 된다(진입가 1,000원이 500원으로 빠지면 dev=+100%). 실측 분포는 자릿수가 다르다: 정상 최대 +12%(ONDO·UNI) vs 오염 최소 +2,552%(MASK) — 그 사이는 표본 0이라 커트를 어디 두든 탐지력은 같고 낮을수록 정상 급락만 죽인다. 회귀 EI8 |
...
| 축적 팩터 유효성 분석 (2026-08-21 고도화 B1-B3) | `analyze_factors.py` — write-only 였던 터치 스냅샷 컬럼 분석 개시: B1 BTC레짐/DVOL/ATR 구간별 승률(Wilson LB), B2 초기반응 IC(score vs ret_4h/12h/24h/72h — 실현 시간축 비교), B3 MTF 정렬·토큰언락 유효성(수급보정 승격 판단 근거). 읽기 전용 CLI, 그룹 n≥20 판정 게이트 | `scripts/analyze_factors.py` |
| 회차 소요시간 기록 (2026-08-21 고도화 A3) | `meta.last_cycle_duration_sec` — 07-27 큐잉 사고류 조용한 지연 추세 관측. show_status 하트비트 행에 `· 소요 Ns` 병기 | `scripts/run_cycle.py` → `scripts/show_status.py` |
...
## 뉴스 요약 — "왜 사야 하는가" 우선 추출 (2026-09-16)

**사용자 요청**: "모닝브리핑 양식 뉴스쪽이 너무 지저분… 내용도 불필요한 게 많고… 핵심주제만 두괄식으로. 내용이 계속 잘리더라."

**진단** — 실제 발송분 15건(`news_digest_queue`) 전수 분석:

| 채널 | 건수 | 패턴 | 문제 |
|------|------|------|------|
| @BitcoinBullets | 4 | `# {코인} 시장 분석` + 현황 + 황소/베어 케이스 + 수사적 질문 + 꼬리말 | 제목이 **심볼+일반명사뿐**인데 80자 컷의 절반을 먹어 정작 분기점이 밀림 |
| @wolfoftrading | 4 | 한두 줄 | 종목 추천 광고·"이 수준을 잃으면 하락"(수준을 안 밝힘) 등 알맹이 없음 |
| @cryptosignals0rg | 4 | 제목 + 불릿 + 본문 | 제목은 두괄식이나 **"IN"이 영어 전치사라 오탐** — 트레이딩 교육글이 코인 뉴스로 실림 |

→ **절반이 노이즈**였고, 정상 글은 제목이 무의미해 내용이 밀려났다.

### 1. 실을 것을 먼저 줄인다 (`notify/news_brief.py`)

| 게이트 | 내용 |
|--------|------|
| 모호 심볼 확대 | `IN`·`ON`·`AT`·`IT`·`BY`·`SO`·`UP`·`OG`·`WIN`·`BEST`·`NEXT` 추가. "IN"이 전치사로 쓰인 교육글을 코인 뉴스로 집던 실측 오탐 |
| 광고 문구 추가 | `찾고 계신가요`·`좋은 선택`·`looking for a coin`·`great pick` 등 — 실측 "투자하기 좋은 코인을 찾고 계신가요? $ZRX는 좋은 선택입니다" |
| **정보 밀도 게이트** | 길이 `< 100자` **이고** 가격다운 수치가 하나도 없으면 스킵. 수치 인정 범위는 `$9,000,000` · 소수(`0.9363`·`249.2`) · 천단위(`2,540`) · 퍼센트. ⚠️ 그냥 `\d` 로 보면 **타임프레임 하나로 통과**한다 — 실측 "$NEOUSDT 업데이트: 30분 / 이 추세선은 곧 기회를 창출할 것"이 '30분' 때문에 살아남았는데 읽어도 할 게 없다 |

### 2. 남은 것에서 "살 이유"를 뽑는다 (`notify/morning_brief.py`)

우선순위 — 위에서부터 첫 매칭:

| # | 경로 | 근거 |
|---|------|------|
| ① | **촉매 문장** `_catalyst_sentence` | 문장별 점수제: **외부 사건 3점**(법안·상원·규제·ETF·SEC / 고래·기관·자금유입·금액) > **예정 이벤트 2점**(상장·메인넷·업그레이드·파트너십·신고가) > **차트 신호 1점**(과매도·돌파·목표가·지지/저항). **2점 이상**만 채택 — 차트 용어 단독은 "왜 지금"에 답하지 못한다. 동점이면 앞선 문장(원문 두괄식 존중) |
| ② | **시나리오 분기** `_scenario_line` | 황소/베어(bull/bear) **머리말은 선택** — 무료 번역기가 회차마다 다르게 옮긴다(실측 "황소 케이스:" · "강세 사례:" · 머리말 없는 "약세:" 세 가지). `강세|약세|상승|하락|bull|bear` + (케이스·사례·시나리오)? + 콜론 + 숫자가 골격. 콜론+숫자가 붙어야 해서 산문 오인 없음(NBC16). 분기 가격 두 개만 → `↑ 0.9000 위 강세 · ↓ 0.8600 아래 약세`. 두 값이 같으면 `122.00 지키면 강세 · 잃으면 약세`(실측 AAVE). 패턴 불일치 시 조용히 다음 경로로 |
| ③ | **제목 줄** | `_is_generic_title` 이 무의미 제목을 걷어낸 뒤 남은 첫 줄. **그 줄에서 끝낸다** — 종전엔 모든 줄을 공백으로 이어 붙여 제목 뒤에 본문이 따라붙어 잘렸다 |
| ④ | 첫 문장 | 위가 전부 없을 때 |
...
| 다듬기 | 내용 |
|--------|------|
| 길이 | `80 → 55자`. 채널 원문이 이미 두괄식이라 제목만 온전히 실으면 되고, 80자는 본문 중간까지 물어 와 "…"로 끊기기만 했다 |
| 절 경계 절단 | 넘칠 때 `_clip_clause` 가 쉼표·`~며`·`~고` 경계에서 끊는다 — 실측 XRP 규제 문장이 "…촉매할…" 처럼 동사 중간에서 잘려 뜻이 끊겼다 |
| 선행 접속사 제거 | 본문 중간 문장을 머리에 올리므로 "또한," 같은 연결어는 군더더기 |
| 무의미 제목 판정 | 심볼·기호·**타임프레임**(`30분`·`4h`)을 걷어낸 뒤 남는 게 일반명사(`시장 분석`·`업데이트`·`전망`…)뿐이면 버린다 |
| 번역 아티팩트 | `# Sol` → `#Sol`, `$ 9,000,000` → `$9,000,000` |

**결과(실측 15건)**: 8건 제거, 7건 채택. 전부 22~34자로 **잘림 없음**.

**회귀**: `NBC1~NBC12`(촉매 우선·접속사·분기 압축·동일값·영문 표기·무의미 제목 3종·절 경계·과추출 방지), `NBB1~NBB5`(본문 정제).
**주의**: 정보 밀도 게이트가 생기면서 종전 NB 픽스처의 더미 문장("…long enough for min length filter")은 "정상 뉴스" 역할을 못 한다 — 픽스처에 가격을 넣어 현실에 맞췄다(NB1·NB10b).

## 진입 시그널 글 필터 (2026-09-14)

| 항목 | 내용 |
|------|------|
| **증상** | `extractor.parse_setup` 이 포맷을 못 읽은 **매매 시그널**이 "셋업 아님"으로 뉴스 경로에 떨어져 시황 뉴스인 척 발송됐다. 실측(09-14 브리핑 큐 id=1): `❤️❤️❤️FREE SIGNAL!❤️❤️❤️ / Instrument: LSKUSDT / My opinion: BUY / Entry: $0.83 / Target: $0.842 / RRR: 1:3` |
| **왜 이제 보였나** | 실시간 발송 때는 하루에 흩어져 눈에 덜 띄었는데, A안으로 브리핑 5줄에 모으니 노이즈가 그대로 드러났다 |
| **구현** | `news_brief._is_trade_setup()` — `_RESULT_PATTERNS`(지난 거래 자랑)와 같은 **점수제**. 고정밀 2점(`free signal`·`instrument:`·`rrr:`·`my opinion:`·`buy zone:`), 보조 1점(`entry:`·`target:`·`sl:`·`tp:`·`leverage:` — **콜론 붙은 라벨 형태만**). 합 **3점 이상**이면 스킵 |
| **문턱이 리캡(2)보다 높은 이유** | 보조 패턴이 라벨이어도 분석글에 한둘은 섞인다(`Key levels — Support: 0.79`). 고정밀 1 + 보조 1, 또는 보조 3개가 모여야 시그널 카드로 본다 |
| **과필터 방지 계약** | 정상 시황글이 걸리면 안 된다 — `entry point for buyers`·`price target of $120,000` 같은 **산문**은 통과. 회귀 NBS4~NBS8 이 이 계약을 못 박는다 |
| **한글 커버** | 번역본 재검사 대비로 한글 라벨도 포함(`기기:`·`내 의견:`·`입장료:`·`경유지:`) |
...
## 수집 단계 진입가 sanity 비대칭화 (2026-09-14, 감사 F3)

| 항목 | 내용 |
|------|------|
| **증상** | `extractor._sanity` 가 대칭 ±60% 라, 2026-09-13 업비트 폴백가를 붙여 **157개 코인이 새로 이 관문에 들어오면서** 정상 셋업까지 자를 위험이 생겼다. 롱 진입가가 현재가보다 크게 아래인 건 "가격이 오른 뒤의 깊은 눌림목 대기"로 정상인데 대칭 커트는 이를 오파싱과 같이 취급한다 |
| **근거** | 실측 NEAR(진입 1,360원 vs 현재 3,138원 = **−56.7%**)는 감시 중인 정상 레벨인데 커트에서 **3.3%p** 차이였다. 같은 날 감시 단계 방어선은 "하단 이탈은 정상"이라 비대칭(`watch_entry_max_below_pct=0`)을 택했는데 수집 단계만 대칭이라 두 관문의 기준이 어긋나 있었다 |
| **구현** | `_sanity` 를 방향별로 나눴다 — 위(`value > 현재가`)는 종전 `max_dev`(60%), 아래는 `sanity_below_max_dev`(**0.90** = 현재가의 10% 미만이면 기각). 그 아래는 눌림목이 아니라 자릿수 오파싱(0.83 → 0.083)이다 |
| **방향** | 이 변경은 수집을 **느슨하게** 만든다 — 의도된 것이다. 새 관문(폴백가)이 정상 셋업을 자르는 부작용을 막는 게 목적이지, 기존 관문을 조이는 게 아니다 |
| **회귀** | 비대칭 7케이스(눌림목 −56.7%/−80% 통과 · 자릿수 오파싱 −95% 기각 · 상단 +50% 통과/+61% 기각 · 오염 +2,552% 기각 · 현재가 미상 통과) |
...
**왜 직접 접나**: 텔레그램에는 CSS 가 없다. 한 줄이 화면 폭을 넘으면 클라이언트가 알아서 접는데 **접힌 줄은 왼쪽 끝(0열)에 붙어** 항목 들여쓰기가 무너진다. `_wrap_indented()` 가 표시 너비 기준으로 미리 나누고 각 줄 머리에 같은 들여쓰기를 넣어 **접힘 자체를 없앤다**.

| 규칙 | 내용 |
|------|------|
| 너비 계산 | `_display_width` 재사용(한글·CJK·이모지 2, 나머지 1). 매크로 캘린더 줄과 같은 유틸 |
| 폭 | `_NEWS_WRAP_W=36`(들여쓰기 포함), 들여쓰기 `_NEWS_INDENT="   "`(코인·채널 줄과 같은 열) |
| 어절 경계 | 공백 단위로 나누되, **한 어절이 폭보다 길면**(긴 URL·붙여쓴 영문) 그 어절만 글자 단위로 쪼갠다 — 안 하면 그 줄이 폭을 넘어 클라이언트 접힘이 다시 일어나 목적을 잃는다 |
| escape 순서 | **접은 뒤** 각 줄에 `html.escape` — 먼저 escape 하면 `&amp;` 같은 엔티티가 줄 경계에서 쪼개져 깨진 문자로 보인다 |
| 분기 줄 표현 단축 | `↑ X 위 강세 · ↓ Y 아래 약세` → `↑ X 강세 · ↓ Y 약세`. "위/아래"는 화살표와 중복이고, 빼야 한 줄(36)에 들어가 접히지 않는다 |

**회귀** MBW1~MBW7(다중 줄 분할·전 줄 동일 들여쓰기·폭 준수·어절 무손실·짧은 줄 미분할·긴 덩어리 글자 분할·빈 문자열).

### 뉴스 2차 필터 — 렌더 직전 재검사 (2026-09-17 실사고)

**사고**: 09-17 아침 브리핑에 시그널 카드가 실렸다 — `📍신호 ID: #2227📍 / 코인: $JUP/USDT (2-5X) / 방향: 긴 / 정지 손실: 0.2160`. 같은 날 07:04 에 이 유형을 막는 필터를 배포했는데도 나갔다.

**원인**: 수집 단계 필터(`news_brief.maybe_send_news_brief`)는 **큐 적재 전에만** 돈다. 큐 적재는 03:23 수집 회차, 필터 배포는 07:04 — **이미 들어간 항목에는 소급되지 않는다**. 필터를 아무리 고쳐도 "고치기 전에 들어온 것"은 그대로 나간다는 구조적 구멍.

**조치**: `morning_brief._is_queued_noise()` — 렌더 직전에 한 번 더 본다. 진입가 sanity 를 수집·감시 두 곳에서 보는 것과 같은 원리(마지막 방어선).

| 규칙 | 내용 |
|------|------|
| 판정 기준 | `news_brief` 의 것을 **그대로 재사용**(모호심볼·광고·청산리캡·시그널카드·정보밀도). 두 곳 기준이 갈리면 어느 쪽이 정본인지 알 수 없어진다. 지연 import 로 순환 방지 |
| 소비 처리 | 걸러진 항목도 **consumed=1** 로 찍는다. 안 그러면 큐에 영원히 남아 매일 재판정하고, `get_news_digest` 가 오래된 순이라 **뒤에 쌓인 정상 뉴스를 계속 굶긴다** |
| 개수 보전 | 상한의 `_NEWS_FETCH_MULT`(=3)배를 꺼내 필터 후 상위 5건. 딱 5건만 꺼내면 그중 3건이 노이즈일 때 2건만 실린다 |
| "외 N건" | **아직 안 본 잔여분** 기준(`count - len(consumed_ids)`). 이번에 소비한 노이즈까지 '남았다'고 세면 숫자가 거짓이 된다 |
| 요약 없음 | 정제 후 남는 게 없으면 항목 자체를 생략(코인명만 보여줄 이유가 없다) |

**회귀** MBQ1~MBQ5. **테스트 픽스처 주의**: 정보 밀도 게이트가 렌더 단계에도 걸리면서 `News body number 0` 류 더미 문장은 "정상 뉴스" 역할을 못 한다 — MB3·MB12 픽스처에 가격을 넣어 현실화했다(실제 뉴스라면 수치가 있는 게 정상).
...
🏁 <b>목표 도달</b> 2건            ← 직전 브리핑 이후 · 최대 8줄, 초과 시 "외 N건"
   AUCTION TP2/3 (진입 +3.2%)
   FIL TP1/8 (진입 +5.3%)
⏳ 대기 레벨 30개 (22개 코인)      ← 앞으로 볼 것을 뒤에 둔다
━━━━━━━━━━━━━━━━━
📰 <b>주요 뉴스</b>                 ← 미소비 큐 · 최대 5건, 초과 시 헤더에 "(외 N건)"
...
| 항목 | 규칙 |
|------|------|
| 🏁 데이터원 | `db.get_tp_hits_since(conn, since)` — `alerts_log kind LIKE 'tp%'` → `levels` 조인. 같은 레벨의 TP1·TP2 는 **최고 단계 1행**으로 접는다 |
| 🏁 조회 창 **(2026-09-14 수리)** | `meta.last_morning_brief_at`(직전 발송 시각) 이후, 없으면 24시간 폴백. meta 가 미래이거나 폴백보다 과거면 신뢰하지 않고 폴백. **종전 `day_kst='어제'` 는 사고였다** — 브리핑이 아침 8~10시에 나가므로 오늘 0~8시 적중이 매번 빠져 다음 날에야 실렸다(실측 09-14 새벽 4건 누락, 최대 32시간 지연). TP 실시간 발송을 끄고 맞바꾼 게 "다음 날 아침 확인"인데 그보다 하루 더 밀리면 맞바꾼 값을 못 받는다. 회귀 MB9~MB11b |
...
| 🏁 상한 | 본문 8줄, 초과 시 `외 N건` |
| 📰 데이터원 **(2026-09-14 수리)** | `db.get_news_digest(conn, limit=5)` — **날짜 조건 없이** 미소비 큐를 오래된 순으로(실시간 발송 순서 재현). 종전 `day_kst='어제'` 필터는 같은 사고 — 새벽에 쌓인 뉴스가 당일 브리핑에서 통째로 빠졌다(실측: 배포 다음 날 브리핑 뉴스 0건, 큐엔 5건 대기). `consumed` 플래그가 이미 중복을 막으므로 날짜로 또 거를 이유가 없다. 회귀 MB12 |
| 📰 상한 | 5건(= 뉴스 글로벌 상한과 동수), 요약 첫 문장 80자 |
| 📰 길이 방어 **(2026-09-14 감사 F1)** | `_fit_telegram` 은 **입력 리스트를 복사**해서 다룬다. 종전엔 제자리 변형 후 같은 객체를 반환해 호출부의 `len(fitted) < len(lines)` 가드가 영구히 False 였고 — 즉 잘려 나간 뉴스까지 `consumed=1` 로 찍혀 **영구 소실**됐다. 가드를 걷고 실린 헤더 줄을 무조건 세어 맞춘다. 요약만 잘리고 헤더만 남은 반쪽 항목은 통째로 제거(내용을 못 본 뉴스를 소비 처리하지 않는다), 블록 전량 제거 시 앞 구분선도 함께 걷는다. 회귀 MB13~MB15 |
| 📰 본문 정제 **(2026-09-14)** | `_news_body()` 가 장식을 걷고 알맹이만 남긴다 — ① 선행 마크다운 헤더 줄(`# FIL 시장 분석`) ② 글자 없는 장식 줄(이모지·기호만) ③ 3자 이상 반복 기호(`➖➖➖`, `---`) **이후** 채널 꼬리말 전부. 종전엔 개행만 공백으로 이어 붙여 제목이 80자 컷의 절반을 먹고 정작 본문이 안 보였다(실측 FIL·XRP). 회귀 NBB1~NBB5 |
