...
- **거래량 급증 2단계 알림(Feature 4)**: 터치 후 72h 감시. 판정은 2026-07-31 교체 —
  최근 1시간 거래대금 > 직전 20시간(완결 60분봉) 평균 × **5.0** + 최근 1h ≥ 0.5억
  (저유동 새벽 위양성 가드). 구 기준(24h > 7일평균 × 3)은 24h 누적이 급증을 반나절
  늦게 반영해 폐기. **감시 제외 밴드**: 현재가가 [진입가 −10%, TP1(없으면 +10%)]
  이탈 시 감시 즉시 종료(밴드는 터치 시점 환율 KRW 고정, 레거시 NULL 행은 시간 만료만)
...
1. **S10 등급 재조정 — 사용자 결정 D1~D5 질문카드 → 개발자 A 구현 투입**
