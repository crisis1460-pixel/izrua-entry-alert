...
- **등급**(2026-08-01 **v3** = S10 안3 확정, D1~D5 승인): 팔로워(1~10) + 근접도(0~20)
  + 목표거리(-6~+12) + 완결성(2~30) + **작성자 실적 가점(0/+5/+10/+15)** = 이론 100
  (실질 상한 87). 등급 임계 S≥85/A≥70/B≥55/C≥40 유지 — S/A 는 검증 작성자 전용 희귀화(D5).
  목표거리 배점표(전 신호 적용) — 2%↓ −6 / 2~3% −4 / 3~5% −2 (감점 불변) /
  5~8% +12 / 8~15% +12 / 15~25% +8 / 25~40% +4 / **40%+ 0(무배점)** — 원거리 TP
  도달률 0% 실측으로 보상 축소(안1). 실적 가점(안2): 자기 DB 종결 n≥5 +
  Wilson 80% 단측 하한 ≥0.55/0.40/0.25 → +15/+10/+5 (`author_track_points`,
  `db.author_closed_stats`) — 등급 목적 = "TP1 도달 가능성 예측"(D1).
  롤백 스위치: `grade_author_points_enabled=False` 면 실적 가점 0 고정(배점표 축소분은 유지).
