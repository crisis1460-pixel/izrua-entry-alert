...
- **OI 급증 알림** (2026-08-08): 진입가 터치와 **무관한** 완전 독립 이벤트 —
  감시 코인(활성 레벨 보유)의 미결제약정(OI)이 1h 전 대비 `oi_spike_pct`
  (기본 15%) 이상 변하면 발동. `oi_history`(수급 판정용 시간당 스냅샷,
  08-07)를 그대로 재사용 — 추가 API 콜 0. 코인당 `oi_spike_cooldown_hours`
  (기본 6h) 쿨다운. 무음 발송, `render_oi_spike_alert`. 본알림 양식(위 §4
  블록)과 무관 — 별도 카드로 옴.
...
🌡️ 자리: 우호 (60일지지·정배열·일38)  ← RSI+MA 5단계 판정(08-08). 52주 블록 아래.
                                    ← 4h RSI 극단(>=70/<=30) 시 (...·4h과열)
                                       (...·4h급락) 태그 병기(08-08, 과열은
                                       최적/우호를 한 단계 강등도 함께)
━━━━━━━━━━━━━━━━━━━━
