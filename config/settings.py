...
SETTINGS = {
    ...
    "tv_fetch_sleep_sec": 6.0,           # 심볼당 요청 최소 간격. 이력: 3.0 이던 2026-07-26
                                          # 실측 쿠키 등록 직후에도 61번째(≈3분)에서 403 →
                                          # 속도 제한으로 판정, 5.0 완충. 그런데 5.0 으로도
                                          # 07-26~08-02 매일 차단 — 61건/3분 실측을 롤링
                                          # 창(~60건/5분)으로 역산하면 5.0s=12건/분이 딱
                                          # 경계라 간헐 차단이 설명된다. 6~9s 지터(평균
                                          # 7.5s=8건/분)로 여유 33% 확보 (2026-08-02).
    "tv_fetch_sleep_max_sec": 9.0,       # 심볼당 요청 최대 간격 — uniform(min,max) 랜덤
    "tv_night_sleep_sec": 12.0,           # KST 0~8시 새벽 최소 간격 (야간 403 완화)
    "tv_night_sleep_max_sec": 18.0,       # KST 0~8시 새벽 최대 간격 (평균 15s=4건/분)
