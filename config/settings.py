...
SETTINGS = {
    # 유니버스 (2026-08-17 재설계): 이 값은 이제 '유니버스 크기'가 아니라 'CG
    # 시총 메타(rank/tier_icon/price_usd) 부여 커버리지'다. build_universe 는
    # Upbit KRW 전 종목(283개)을 유니버스에 편입하고, 이 값 안(top-N)에 매칭
    # 되는 코인만 rank 등 메타를 부여. 밖은 rank=None, tier_icon='·' 로 편입.
    # 즉 이 값을 올려도 유니버스는 안 커지고 tier 아이콘 커버만 확장.
    # 되돌리기(옛 top-N ∩ KRW 방식으로): collector/coingecko.build_universe 를
    # 이 커밋(97e6e6a9) 이전 로직으로 리버트 필요 — 이 값 변경만으론 안 됨.
    "universe_top_n": 300,               # CoinGecko 시총 상위 N (메타 커버리지)
    ...
    # ── TP 없는 글 터치 알림 배제 (2026-09-22 D1/R1) ────────────────────
    # 실측 근거(plan_2026-09-22_최종버전_종합검토.md §2-1 D1): TP·SL 둘 다 파싱
    # 되지 않아 판정축이 judgment_mode='timeboxed' 로 떨어지는 글은 실현수익
    # 평균 -2.37% / PF 0.60(이상치 제외, n=78) — 세 모드 중 **유일한 음의
    # 기대값**이고 발송분 31건의 PF 는 0.13. 09-13 C 컷 상향이 이미 사실상
    # 차단(발송 31→1건)하고 있으나, 컷을 되돌리면 재발하므로 규칙으로 못박는다.
    # 따라서 알림량 영향은 ≈0 이고 v6 관찰과 겹치지 않는다(plan §3 순서 원칙).
    # 적용 범위: **터치 본알림만**(예고는 미적용). 터치 기록·판정·MFE 추적은
    # 그대로 — 데이터는 계속 쌓이고 알림만 억제된다. 되돌리기는 False 한 줄.
    "alert_exclude_no_tp": True,
    ...
    # 거래대금 순위 1-20위 군의 승률 49.3%·PF 1.32(n=146) vs 100위+ 61.9%·PF 1.90
    # (n=84). 공정 승률 35.7% vs 50.0%(LB80 42.9). tp_sl 층에서도 방향 유지.
    # "대형 코인의 지지선 글은 이미 모두가 보고 있는 자리"라는 해석.
    # **터치 재채점에서만** 실린다(수집 시점엔 순위가 없어 None → 0점) —
    # 지연 감점(v6 Q1)과 완전히 같은 배선. score_breakdown 키 "vol_rank".
    #
    # ⚠️ **2026-10-06 까지 False 고정.** v6 지연 감점 2주 관찰(~10-06)이 끝나기
    # 전에는 켜지 않는다 — 09-13 교훈 "알림량을 바꾸는 변경은 겹치지 않는다"
    # (plan §3 순서 원칙). 그 날짜 이후 True 한 줄로 활성화, 되돌리기도 한 줄.
    "grade_volume_rank_enabled": False,
    "grade_volume_rank_top_n": 20,         # 이 순위 **이하**(1~N)면 감점
    # -6: 종결 터치 표본(n=214, 운영 DB 2026-09-22) 시뮬 기준 추천값.
    # 후보 비교(1-20위 군 C 이상 통과율 감소 / 잘려나간 건의 승률):
    #   -3 → 6.2% / 0.0%   -4 → 8.3% / 0.0%   -6 → 14.6% / 14.3%
    #   -9 → 25.0% / 41.7%  -10 → 33.3% / 37.5%
    # 기획서 목표(25~35% 감소)는 -9~-10 이 충족하나 그 구간부터 **승률 40%대의
    # 정상 신호를 자르기 시작**한다(전체 통과군 승률도 -6 의 65.4% 가 최대).
    # 상세표는 izrua_company/note_2026-09-22_volrank_notp.md.
    "grade_volume_rank_penalty": -6,
    ...
    # ── 주간 리포트 v2 (2026-09-22 R4 — 지표 나열형 → 의사결정형 개편) ──
    # 목표 길이 3,500자. 넘으면 _fit_weekly 가 구분선/줄 경계에서 자르고 한 줄로
    # 알린다(텔레그램 4096 분할 발송 전에 리포트 스스로 예산을 지킨다 — 두 통으로
    # 쪼개진 주간 리포트는 읽히지 않는다).
    "weekly_report_max_chars": 3500,
    # 요약·관찰 줄의 소표본 임계. 이 미만이면 화살표를 생략하고 "(n=…, 참고)" 로만
    # 표기한다(소표본에서 화살표는 노이즈를 방향으로 착각하게 만든다).
    "weekly_report_min_n": 10,
    # 관찰 줄 최대 개수 (TradeZella 주간 리뷰 프레임의 "관찰 3개").
    "weekly_report_observations": 3,
    # 작성자 랭킹 표시 상한 (수학·정렬은 불변, 표시만 상위 N).
    "weekly_report_top_authors": 5,
    # 관찰·집계 누적 창(일). 1주 창은 그룹당 n≥10 을 거의 못 넘어 4주 누적으로 본다.
    "weekly_report_pool_days": 28,
    # ④ 다음 판단 마일스톤 목표치 (plan_2026-09-22 §3 라운드2 S1·S2 트리거).
    "weekly_report_milestone_v6": 150,     # v6 지연감점 평가 — 종결 150건
    "weekly_report_milestone_mfe": 50,     # MFE/MAE e-ratio — 종결 50건
    ...
    # 수집 단계 진입가 sanity **하단** 허용폭 (2026-09-14, 감사 F3).
    # 상단은 parse_setup(max_dev=0.60) 이 맡고, 이 값은 "진입가가 현재가보다 아래"
    # 방향만 본다. 위아래가 뜻하는 바가 달라서다 — 위는 '이미 지나간 자리/오파싱'
    # 이지만 아래는 **가격이 오른 뒤의 깊은 눌림목 대기**로 정상이다.
    # 대칭 ±60% 였다면 실측 NEAR(진입 1,360원 vs 현재 3,138원 = −56.7%, 감시 중인
    # 정상 레벨)가 커트에서 3.3%p 차이로 아슬아슬했다. 같은 날 감시 단계 방어선
    # (watch_entry_max_below_pct=0)도 하단은 검사하지 않기로 했는데 수집 단계만
    # 대칭이라 두 관문의 기준이 어긋나 있었다.
    # 0.90 = 현재가의 10% 미만이면 기각 — 그 아래는 눌림목이 아니라 자릿수
    # 오파싱(0.83 → 0.083)이다. 1.0 이상으로 두면 하단 검사를 사실상 끈다.
    "sanity_below_max_dev": 0.90,
    ...
    # 2026-09-14 상향(60 → 300, 감사 F2): 60% 는 **정상 급락을 오파싱으로 오인**한다.
    # 이 방어선은 터치 판정 루프보다 앞에 있어서, 회차가 길게 끊긴 뒤(09-13 러너
    # 291분 정지 같은 상황) 가격이 진입가 아래로 크게 빠진 레벨은 '소급 터치'가
    # 아니라 'entry_insane 만료'가 된다 — 진입가 1,000원짜리가 500원으로 빠지면
    # dev=+100% 라 60% 커트에 걸리는데, 그건 잡아야 할 오파싱이 아니라 **놓치면
    # 안 되는 터치**다. 코인이 하루에 반토막 나는 건 드문 일이 아니다.
    # 실측 분포상 두 집단은 자릿수가 다르다: 정상 레벨 최대 상단 이탈 +12%(ONDO·UNI)
    # vs 오염 레벨 최소 +2,552%(MASK). 60~2,552% 는 표본 0의 미검증 구간이라
    # 커트를 그 사이 어디에 둬도 오염 탐지력은 같고, 낮을수록 정상 급락만 더 죽인다.
    # 300 이면 가격이 1/4 토막 나도 터치 판정이 우선이고 오염은 그대로 걸린다.
    "watch_entry_max_above_pct": 300.0,  # 진입가가 현재가보다 위 — 오파싱 탐지축
    ...
    # 2026-08-18 사용자 결정: 5.0 → 8.0 상향. 근거: 조용한 코인은 20h 평균 자체가
    # 너무 낮아 소폭 거래 증가만으로도 5x 돌파. 8x는 업계 RVOL 기준 "극단적 급증"
    # 수준으로, 진짜 펌핑·뉴스 주도 거래량만 포착.
    "volume_spike_multiplier": 8.0,       # 최근 1h > 20h 평균 × 이 배수 → 급증 판정
    ...
    # 2026-08-18 사용자 결정: 1억 → 5억 상향. 근거: 조용한 코인은 20h 평균이
    # 워낙 낮아 절대금액도 높여야 의미 있는 급증만 잡힘. 시간당 5억은 중소형
    # 알트에서도 뉴스·펌핑 수준의 거래대금.
    "volume_spike_min_krw_60m": 500_000_000,
    ...
    # 2026-08-17 완화(15→10) 후 2026-08-21 사용자 결정으로 재강화: 10% 단독은
    # 소형 OI($14~16M — VVV/POL/IMX/CYBER)의 일상 출렁임까지 잡아 2.5건/일
    # 발동(불장 아닌데 과다). 파생 관례상 소형 OI 는 절대 규모 필터가 정석 —
    # 15% + 현재 OI ≥ $30M 이중 게이트로 0.5건/일 수준(2일 백테스트),
    # AAVE($58M)급 메이저 유입만 통과. 쿨다운 6h 유지.
    "oi_spike_pct": 15.0,           # 1h 전 대비 |변화율| 이 이상이면 급증 판정
    "oi_spike_min_oi_usd": 30_000_000,  # 현재 OI 절대 하한 — 소형 노이즈 차단
