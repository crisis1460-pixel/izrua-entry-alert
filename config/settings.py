...
SETTINGS = {
    ...
    # 2026-08-18 사용자 결정: 3.0 → 5.0 복원. 근거: 08-17 완화(5→3) 후 AVAX 등
    # 평상시 거래량 소폭 증가(3.7x)만으로 알림 발동 — 20h 평균이 낮은 코인에서
    # 의미 없는 급증이 잡힘. 원래의 5x(극단 수준)로 복원해 진짜 펌핑만 포착.
    "volume_spike_multiplier": 5.0,       # 최근 1h > 20h 평균 × 이 배수 → 급증 판정
    ...
    # 2026-08-18 사용자 결정: 1억 → 3억 상향. 근거: multiplier 5x 복원과 함께
    # 절대 하한도 강화. AVAX 사례(2.7억/3.7x) 같은 소규모 급증 차단. 진짜
    # 펌핑은 중소형 알트도 시간당 수억 이상.
    "volume_spike_min_krw_60m": 300_000_000,
