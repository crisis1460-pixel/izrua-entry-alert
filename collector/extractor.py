...
def _grab_after(label_pat, text: str) -> list:
    ...
    ⚠️ 창 크기가 비대칭이다 — _LADDER 만 '첫 숫자가 있는 줄의 끝'까지 보고,
    _RANGE·_SINGLE 은 종전대로 30자다 (2026-07-27 교차감사 B-M1 수리). 이유:
    ...
    for m in label_pat.finditer(text):
        ...
        if sng:
            ...
            lad = _LADDER.search(text[m.end(): len(text) if _nl < 0 else _nl])
